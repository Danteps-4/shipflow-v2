...
import { isValidHref } from "@/lib/navGroups";
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const body = await req.json() as { role?: string; modules?: string[]; linkAccess?: string[] };
  const role = body.role === "admin" ? "admin" : "member";
  const modules = (body.modules ?? []).filter(isModuleKey);
  const linkAccess = body.linkAccess?.filter(isValidHref);

  const updated = updateUserAccess(params.id, { role, modules, linkAccess });
  if (!updated) return NextResponse.json({ error: "Usuario no encontrado" }, { status: 404 });

  return NextResponse.json({
    user: {
      id: updated.id, name: updated.name, email: updated.email,
      role: updated.role, modules: updated.modules, linkAccess: updated.linkAccess,
    },
  });
}
