...
export interface PedidoSeleccionado {
  ...
  pedidoProductos: ProductoPedidoSeleccionado[];
}

// ─── Helpers de normalización ─────────────────────────────────────────────────

function tnDireccion(addr: TnShippingAddress | null): string {
  if (!addr) return "";
  const provincia = typeof addr.province === "string" ? addr.province : addr.province?.name ?? "";
  const partes = [
...
function normalizeTnOrder(o: TnOrderResumen): PedidoSeleccionado {
  return {
    ...
    clienteDireccion: tnDireccion(o.shipping_address),
    pedidoTotal: o.total ? Number(o.total) : null,
    pedidoMoneda: o.currency || "ARS",
    pedidoFecha: o.created_at,
    pedidoEstado: `${o.status} · pago: ${o.payment_status} · envío: ${o.shipping_status}`,
    pedidoTransportista: o.shipping_carrier_name || (typeof o.shipping_option === "string" ? o.shipping_option : o.shipping_option?.name) || "",
    pedidoTracking: o.shipping_tracking_number || o.fulfillments?.[0]?.tracking_number || "",
...
function normalizeMlOrder(o: MlOrderResumen): PedidoSeleccionado {
  ...
  return {
    ...
    clienteDni: "",
    clienteDireccion: "",
    pedidoTotal: o.total_amount ?? null,
    pedidoMoneda: o.currency_id || "ARS",
    pedidoFecha: o.date_created || "",
    pedidoEstado: o.status,
    pedidoTransportista: "",
    pedidoTracking: "",
...
export default function TicketOrderPicker({
  onSelect, onClose,
}: {
  onSelect: (pedido: PedidoSeleccionado) => void;
  onClose: () => void;
}) {
  ...
  function onBuscarTn(value: string) {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setTnError(null);
      try {
        const res = await fetch(`/api/tickets/orders?canal=tiendanube&q=${encodeURIComponent(value)}&per_page=8`);
        if (res.ok) {
  ...
  async function fetchMl(offset: number) {
    ...
    setMlError(null);
    try {
      const res = await fetch(`/api/tickets/orders?canal=mercadolibre&offset=${offset}&limit=${ML_LIMIT}`);
      if (res.ok) {
